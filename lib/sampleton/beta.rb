...
class Sampleton::Beta
  ...
  def compare(a, b)
    ...
    raise "Incompatible profile lengths: #{a.size} != #{b.size}." unless
            a.size==b.size
    send(index, a, b)
  end
  ...
  def jaccard(a, b)
    a_ind = a.map{ |i| i>0 }
    b_ind = b.map{ |i| i>0 } 
    n = (0 .. (a.size-1)).map{ |i| (a_ind[i] and b_ind[i]) ? 1 : 0 }.inject(:+)
    u = (0 .. (a.size-1)).map{ |i| (a_ind[i]  or b_ind[i]) ? 1 : 0 }.inject(:+)
    return n.to_f/u
    #b = bray_curtis(a, b)
    #return 2.0*b/(1+b)
  end
  ...
  def smc(a, b)
    a_ind = a.map{ |i| i>0 ? 1 : 0 }
    b_ind = b.map{ |i| i>0 ? 1 : 0 }
    m00 = (0 .. (a.size-1)).map{ |i| a_ind[i]==0 and b_ind[i]==0 ? 1 : 0 }
    m11 = (0 .. (a.size-1)).map{ |i| a_ind[i]==1 and b_ind[i]==1 ? 1 : 0 }
